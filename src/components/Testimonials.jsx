...
const Testimonials = () => {
  return (
    ...
    className='container mx-auto py-10 lg:px-32 w-full overflow-hidden' id='Testimonials'>
        ...
        <p className='text-center text-gray-500 mb-12 max-w-80 mx-auto'>Where dreams meet reality: Stories of finding home with us</p>
        <div className='flex flex-wrap justify-center gap-8'>
            {testimonials.map((testimonial, index) => (
                <div className='max-w-[340px] border shadow-lg shadow-purple-100 rounded px-8 py-12 text-center' key={index}>
                    <img className='w-20 h-20 rounded-full mx-auto mb-4' src={testimonial.image} alt={testimonial.alt}/>
                    <h2 className='text-xl text-gray-700 font-medium'>{testimonial.name}</h2>
                    <p className='text-gray-500 mb-4 text-sm'>{testimonial.title}</p>
                    <div className='flex justify-center gap-1 text-purple-700 mb-4'>
